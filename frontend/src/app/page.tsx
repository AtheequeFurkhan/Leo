...
const workspaceHighlights = [
  {
    ...
    description: "Executive summary, risks, and recommended bets with stronger visual hierarchy.",
  },
  {
    icon: BarChart3,
    title: "Artifact workspace",
    description: "Competitor scorecards, pricing intelligence, charts, and maps in modern cards.",
  },
  {
    icon: Compass,
    title: "Follow-up flow",
    description: "Quick prompt chips keep the conversation moving without losing analysis context.",
  },
];
        ...
        {messages.length === 0 ? (
          <div className="space-y-8 py-10">
            <section className="grid gap-6 lg:grid-cols-[1.2fr,0.8fr]">
              <Card className="overflow-hidden border-primary/10 bg-[radial-gradient(circle_at_top_left,rgba(99,102,241,0.18),transparent_38%),linear-gradient(180deg,rgba(255,255,255,0.08),rgba(255,255,255,0.03))]">
                <CardContent className="p-8 sm:p-10">
                  <Badge variant="purple" className="normal-case tracking-normal">
                    Premium AI workspace
                  </Badge>
                  <h2 className="mt-5 max-w-2xl text-4xl font-semibold tracking-tight text-slate-50 sm:text-5xl">
                    Turn open-ended market questions into strategic intelligence.
                  </h2>
                  <p className="mt-4 max-w-2xl text-base leading-8 text-slate-300">
                    Leo coordinates specialist agents to assemble executive summaries, pricing intelligence, competitor maps, and follow-up opportunities for the selected company.
                  </p>
                  <div className="mt-8">
                    <FollowupChips chips={suggestedChips} onSelect={handleSubmit} align="left" />
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="grid gap-4 p-6">
                  <div className="rounded-[24px] border border-white/8 bg-black/10 p-5">
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="text-xs uppercase tracking-[0.22em] text-slate-500">Analysis target</div>
                        <div className="mt-2 text-lg font-medium text-slate-100">{product.name}</div>
                      </div>
                      <CircleDashed className={`h-5 w-5 ${isProcessing ? "animate-spin text-indigo-300" : "text-slate-500"}`} />
                    </div>
                    <div className="mt-2 text-sm text-slate-400">{product.url}</div>
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
                    <div className="rounded-[24px] border border-white/8 bg-black/10 p-5">
                      <div className="text-xs uppercase tracking-[0.22em] text-slate-500">Mode</div>
                      <div className="mt-2 text-sm text-slate-200">{useMock ? "Demo mode" : "API-backed analysis"}</div>
                    </div>
                    <div className="rounded-[24px] border border-white/8 bg-black/10 p-5">
                      <div className="text-xs uppercase tracking-[0.22em] text-slate-500">Output types</div>
                      <div className="mt-2 text-sm text-slate-200">Briefs, findings, charts, matrices, risks, and pricing tables</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
                      ...
                      {message.response.agent_outputs
                        ...
                        .map((output) => (
                          <div
                            ...
                            className="rounded-2xl border border-rose-400/15 bg-rose-400/8 px-4 py-3 text-sm text-rose-200"
                          >
                            {output.agent_name} returned {output.status}
                            {output.errors.length > 0 ? `: ${output.errors[0]}` : ""}
                          </div>
                        ))}
                      {index !== messages.length - 1 ? <Separator className="mt-8" /> : null}
                    </>
                  ) : null}
                </div>
              ))}
                ...
                <Card className="bg-white/[0.03]">
                  <CardContent className="p-6">
                    <div className="mb-4 flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-slate-100">Suggested next questions</span>
                    </div>
                    <FollowupChips chips={suggestedChips} onSelect={handleSubmit} align="left" />
                  </CardContent>
                </Card>
